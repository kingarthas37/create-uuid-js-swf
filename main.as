package  {
	...
	public class main extends MovieClip {
		...
		public function main() {
			 ...
			 //var uuid  = loaderInfo.parameters['uuid'] !== undefined ? loaderInfo.parameters['uuid'] : 'nouuid';
			
			Security.allowDomain('*');
			Security.allowInsecureDomain("*");
			ExternalInterface.addCallback("callExternalInterface",callExternalInterface);
			  
			
			 mySO = SharedObject.getLocal("_tj");
			 
			 ExternalInterface.call('_tj_user_info_callback');
			  
		}
		
		
		
		function callExternalInterface():void{
			
			//如果没有uuid，第一次生成，创建uuid和date
			if(!mySO.data.uuid) {
			 	createInfo();
			}
			...
			//如果有参数，表示重置data
			if(arguments.length) {
				mySO.data.datas = base64.encode(arguments[0]);
			}
			
			ExternalInterface.call('_tj_user_info',mySO.data.uuid,mySO.data.ticket,base64.decode(mySO.data.datas));
		...
		private function createInfo():void {
			...
			var _uuid:String = getUuid();
			_uuid = _uuid.replace(/\-/g,'');
			
			var _ticket:String = getTicket();
			
			mySO.data.uuid = _uuid;
			mySO.data.ticket = _ticket;
			mySO.data.datas = '';
		 ...
		 //生成ticket
		private function getTicket():String {
